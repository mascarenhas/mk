...
_M._NAME = "mk.routes"
_M._VERSION = "0.1"
_M._COPYRIGHT = "Copyright (C) 2019 Fabio Mascarenhas"
_M._DESCRIPTION = "Route parser/builder for MK"
...
local param =
  ...
  function(prefix, name)
    local extra = {
      inner = (lpeg.P(1) - lpeg.P("/")) ^ 1,
      close = lpeg.P("/") + lpeg.P(-1)
    }
    return {
      cap = lpeg.Carg(1) * re.compile([[ [/.] {%inner+} &(%close) ]], extra) / function(params, item, delim)
          params[name] = util.decode_uri_part(item)
        end,
...
local splat = re.compile [[ {[/.]} {'**'} &('/' / !.) ]] / function(prefix)
...
local function fold_caps(cap, acc)
  ...
  elseif cap.cap == "*" then
    return {
      cap = (lpeg.Carg(1) * (lpeg.P(cap.prefix) * lpeg.C((lpeg.P(1) - acc.clean) ^ 0)) ^ -1 / function(params, splat)
          if not params.splat then
            params.splat = {}
          end
          if splat and splat ~= "" then
            params.splat[#params.splat + 1] = util.decode_uri_part(splat)
          end
        end) * acc.cap,
...
local route =
  lpeg.Ct((param + opt_param + splat + rest) ^ 1 * lpeg.P(-1)) /
  function(caps)
    return foldr(caps, fold_caps, {cap = lpeg.P("/") ^ -1 * lpeg.P(-1), clean = lpeg.P("/") ^ -1 * lpeg.P(-1)}), foldl(
      caps,
      fold_parts,
      {}
    )
