...
do
   local r = R('/download/**/:filename')
   local t = r:match('/download/path/to/file.xml')
   assert(#t.splat == 1)
   assert(t.splat[1] == "path/to")
   assert(t.filename == "file.xml")
   assert(r:build{ splat = { "path/to" }, filename = "file.xml" } == "/download/path/to/file.xml")
end
...
do
   ...
   assert(r:build{ foo = "hello world", splat = { "how are you" } } == '/hello%20world/how%20are%20you')
end 
